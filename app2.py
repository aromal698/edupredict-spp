...
# UI
# ============================================================
from db import register_student as db_register_student, get_students as db_get_students, get_student as db_get_student, register_tutor as db_register_tutor, save_student_marks as db_save_student_marks, get_all_marks as db_get_all_marks, delete_mark_record as db_delete_mark_record, save_smart_card as db_save_smart_card, get_smart_cards as db_get_smart_cards, add_audit_log as db_add_audit_log
...
def load_registrations():
    ...
    if not rows: return empty_registration_df()
    return pd.DataFrame([{"Username":r.get("university_id",""),"University_ID":r.get("university_id",""),"Student_Name":r.get("student_name",""),"Department":r.get("department",""),"Semester":r.get("semester",""),"Tutor_Username":r.get("registered_by",""),"Registered_Time":r.get("registered_at","")} for r in rows], columns=REGISTRATION_COLUMNS)

def save_registrations(df):
    for _,r in df.iterrows():
        db_register_student({"university_id":r.get("University_ID",""),"student_name":r.get("Student_Name",""),"department":r.get("Department",""),"semester":r.get("Semester",""),"registered_by":r.get("Tutor_Username","")})

def log_action(role, username, action, university_id="", department="", semester="", details=""):
    try: db_add_audit_log(username, role, action, university_id, department, semester, details)
    except Exception as exc: st.warning(f"Supabase audit log error: {exc}")

def register_student(username, uid, name, department, semester, tutor_username):
    uid,name=str(uid).strip(),str(name).strip(); department,semester,tutor_username=str(department).strip(),str(semester).strip().upper(),str(tutor_username).strip()
    if not uid or not name or not department or not semester: return False,"Enter University ID, Student Name, Department and Semester."
    if department not in DEPARTMENTS or semester not in SEMESTERS: return False,"Select a valid B.Tech Department and Semester."
    try:
        if db_get_student(uid) and db_get_student(uid).get("active",True): return False,"This University ID is already registered."
        db_register_student({"university_id":uid,"student_name":name,"department":department,"semester":semester,"registered_by":tutor_username,"active":True})
    ...
    except Exception as exc: return False,f"Supabase registration failed: {exc}"

def find_registered_student(uid):
    r=db_get_student(uid)
    if not r or not r.get("active",True): return None
    return {"Username":r.get("university_id",""),"University_ID":r.get("university_id",""),"Student_Name":r.get("student_name",""),"Department":r.get("department",""),"Semester":r.get("semester",""),"Tutor_Username":r.get("registered_by",""),"Registered_Time":r.get("registered_at","")}

def find_registered_credentials(username, uid):
    return find_registered_student(uid)

def load_reports():
    rows=db_get_all_marks()
    ...
    for r in rows:
        ...
        student=find_registered_student(r.get("university_id","")) or {}
        out.append({"Username":r.get("university_id",""),"Student_Name":student.get("Student_Name",""),"University_ID":r.get("university_id",""),"Semester":r.get("semester",""),"Department":r.get("department",""),"Subjects_JSON":json.dumps(subs),"Created_Time":r.get("submitted_at","")})
    return pd.DataFrame(out,columns=REPORT_COLUMNS)
...
def upsert_report(report):
    subs=report.get("Subjects_JSON",[])
    if isinstance(subs,str):
        try: subs=json.loads(subs)
        except Exception: subs=[]
    tutor=str(st.session_state.get("username",""))
    db_save_student_marks({"university_id":report.get("University_ID",""),"department":report.get("Department",""),"semester":report.get("Semester",""),"tutor_name":tutor,"subjects":subs})
    log_action("Tutor",tutor,"Student Mark Submission",report.get("University_ID",""),report.get("Department",""),report.get("Semester",""),"Marks submitted")


def delete_student_record(report):
    try:
        db_delete_mark_record(report.get("University_ID",""),report.get("Created_Time",""))
        log_action("Tutor",st.session_state.get("username",""),"Student Mark Record Deleted",report.get("University_ID",""),report.get("Department",""),report.get("Semester",""),"Deleted selected mark submission")
        return True,"Deleted selected student mark record."
    except Exception as exc: return False,f"Supabase delete failed: {exc}"
...
def load_smart_cards():
    ...
    if not rows: return pd.DataFrame(columns=SMART_CARD_COLUMNS)
    return pd.DataFrame([{"Registration_ID":r.get("registration_id",""),"Student_Name":r.get("name",""),"DOB":r.get("dob","") or "","Blood_Group":r.get("blood_group",""),"Address":r.get("address",""),"PIN_Code":r.get("pin_code",""),"Studied_College":r.get("studied_college",""),"Department":r.get("department",""),"Semester":r.get("semester",""),"CGPA":r.get("cgpa","") if r.get("cgpa") is not None else "","University_Name":r.get("university_name",""),"University_ID":r.get("university_id","") or "","Submitted_Time":r.get("submitted_at","")} for r in rows],columns=SMART_CARD_COLUMNS)
...
def teacher_login():
    ...
    with st.form("teacher_login_form"):
        username=st.text_input("Tutor Username"); password=st.text_input("Password",type="password")
    ...
    if login:
        ...
        if account and account["password"]==password:
            st.session_state.logged_in=True; st.session_state.role="teacher"; st.session_state.username=username.strip(); st.session_state.teacher_department=account["department"]; st.session_state.active_department=account["department"]; st.session_state.teacher_menu_view="menu"
            try: db_register_tutor({"tutor_id":username.strip(),"tutor_name":username.strip(),"department":account["department"],"semester":"","credit_score":0,"active":True})
            except Exception: pass
            log_action("Tutor",username.strip(),"Tutor Login","",account["department"],"","Successful tutor login")
            st.session_state.page="teacher_portal"; st.rerun()
        else: st.error("Invalid tutor username or password.")


def student_login():
    login_shell("Student Login", "Use the University ID registered by your tutor")
    st.info("💡 Enter the University ID given by your tutor. No student username or password is required.")
    ...
    if login:
        ...
        if not registered:
            st.error("❌ No tutor-registered profile found for this University ID."); return
        latest_report=find_student(registered["University_ID"],registered["University_ID"])
        st.session_state.logged_in=True; st.session_state.role="student"; st.session_state.username=str(registered["University_ID"]).strip(); st.session_state.student_profile=registered
        log_action("Student",registered["University_ID"],"Student Login",registered["University_ID"],registered.get("Department",""),registered.get("Semester",""),"Successful student login")
...
def tutor_portal():
    ...
    tutor_id = st.session_state.get("username", "")
    with st.expander("👨‍🏫 Tutor Profile & Credit Score", expanded=False):
        tutor_name = st.text_input("Tutor Name", value=tutor_id, key="tutor_name_profile")
        tutor_credit = st.number_input("Tutor Credit Score /10", min_value=0.0, max_value=10.0, value=0.0, step=0.1, key="tutor_credit_profile")
        if st.button("💾 Save Tutor Profile", key="save_tutor_profile"):
            try:
                db_register_tutor({"tutor_id":tutor_id,"tutor_name":tutor_name.strip() or tutor_id,"department":assigned_department,"semester":"","credit_score":tutor_credit,"active":True})
                log_action("Tutor",tutor_id,"Tutor Profile Updated","",assigned_department,"",f"Tutor={tutor_name}; Credit={tutor_credit}/10")
                st.success("✅ Tutor profile saved. Principal can now see the tutor and credit score.")
            except Exception as exc: st.error(f"Could not save tutor profile: {exc}")
    st.markdown("""
    ...
    if st.session_state.get("teacher_menu_view") == "registration":
        ...
        st.caption("✨ Students sign in using University ID only. No student username is required.")
        ...
        if submitted:
            ok, msg = register_student(uid, uid, name, assigned_department, semester, st.session_state.get("username", ""))
