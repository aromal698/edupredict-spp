...
def load_registrations():
    try:
        ...
        if rows:
            ...
            return out.reindex(columns=REGISTRATION_COLUMNS, fill_value="")
...
def log_action(role, username, action, university_id="", department="", semester="", details=""):
    try:
        db.add_audit_log(username, role, action, university_id, department, semester, details)
    except Exception:
        pass
    row = pd.DataFrame([{
    ...
    try:
        old = pd.read_csv(AUDIT_LOG_FILE) if os.path.exists(AUDIT_LOG_FILE) else pd.DataFrame(columns=AUDIT_COLUMNS)
        for col in AUDIT_COLUMNS:
            if col not in old.columns: old[col] = ""
        pd.concat([old[AUDIT_COLUMNS], row], ignore_index=True).to_csv(AUDIT_LOG_FILE, index=False)
    except Exception:
        pass
...
def register_student(username, uid, name, department, semester, tutor_username):
    ...
    if not username or not uid or not name or not department or not semester:
        return False, "Enter Student Name, University ID, Department and Semester."
    if department not in DEPARTMENTS or semester not in SEMESTERS:
        return False, "Select a valid B.Tech Department and Semester."
    try:
        existing = db.get_student(uid)
        if existing:
            return False, "This University ID is already registered."
        db.register_student({
            "university_id": uid, "student_name": name, "department": department,
            "semester": semester, "studied_college": "", "registered_by": tutor_username, "active": True
        })
    except Exception as exc:
        return False, f"Could not save student to Supabase: {exc}"
    row = pd.DataFrame([{
        "Username": username, "University_ID": uid, "Student_Name": name,
        "Department": department, "Semester": semester, "Tutor_Username": tutor_username,
        "Registered_Time": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    }], columns=REGISTRATION_COLUMNS)
    try:
        old = load_registrations()
        save_registrations(pd.concat([old, row], ignore_index=True))
    except Exception:
        pass
...
def load_reports():
    try:
        ...
        for r in rows:
            ...
            out.append({
                "Username": r.get("tutor_name", ""),
                "Student_Name": r.get("university_id", ""),
                "University_ID": r.get("university_id", ""),
                "Semester": r.get("semester", ""),
                "Department": r.get("department", ""),
                "Subjects_JSON": json.dumps(subjects, ensure_ascii=False),
                "Created_Time": r.get("submitted_at", ""),
            })
        return pd.DataFrame(out, columns=REPORT_COLUMNS) if out else empty_df()
    except Exception:
        if not os.path.exists(REPORT_FILE): return empty_df()
        try:
            df=pd.read_csv(REPORT_FILE)
            for col in REPORT_COLUMNS:
                if col not in df.columns: df[col]=""
            return df[REPORT_COLUMNS]
        except Exception: return empty_df()
...
def upsert_report(report):
    try:
        subjects = report.get("Subjects_JSON", "[]")
        if isinstance(subjects, str): subjects=json.loads(subjects)
        db.save_student_marks({
            ...
            "tutor_name": str(report.get("Username", "")).strip(),
            "subjects": subjects,
        })
    except Exception as exc:
        raise RuntimeError(f"Supabase mark save failed: {exc}")
    try:
        df=load_reports()
        local=pd.DataFrame([report], columns=REPORT_COLUMNS)
        save_reports(pd.concat([df, local], ignore_index=True))
...
def delete_student_record(report):
    try:
        db.delete_mark_submission(report.get("University_ID", ""), report.get("Department", ""), report.get("Semester", ""), report.get("Created_Time") or None)
    except Exception as exc:
        return False, f"Supabase deletion failed: {exc}"
    try:
        df=load_reports()
        uid=str(report.get("University_ID", "")).strip(); dept=str(report.get("Department", "")).strip(); sem=str(report.get("Semester", "")).strip(); created=str(report.get("Created_Time", "")).strip()
        mask=(df["University_ID"].astype(str).str.strip().eq(uid)&df["Department"].astype(str).str.strip().eq(dept)&df["Semester"].astype(str).str.strip().str.upper().eq(sem.upper())&df["Created_Time"].astype(str).str.strip().eq(created))
        if mask.any(): save_reports(df.loc[~mask].copy())
    except Exception: pass
    return True, "Deleted the selected student submission from the shared database."
...
def load_smart_cards():
    try:
        ...
        return pd.DataFrame(mapped, columns=SMART_CARD_COLUMNS) if mapped else pd.DataFrame(columns=SMART_CARD_COLUMNS)
    except Exception:
        if not os.path.exists(SMART_CARD_FILE): return pd.DataFrame(columns=SMART_CARD_COLUMNS)
        try:
            df=pd.read_csv(SMART_CARD_FILE,dtype=str).fillna("")
            for col in SMART_CARD_COLUMNS:
                if col not in df.columns: df[col]=""
            return df[SMART_CARD_COLUMNS]
        except Exception: return pd.DataFrame(columns=SMART_CARD_COLUMNS)

def save_smart_card(row):
    try:
        db.save_smart_card({
            "registration_id":str(row.get("Registration_ID","")).strip(),
            "university_id":str(row.get("University_ID","")).strip() or None,
            "name":str(row.get("Student_Name","")).strip(), "dob":row.get("DOB") or None,
            "blood_group":str(row.get("Blood_Group","")), "address":str(row.get("Address","")),
            "pin_code":str(row.get("PIN_Code","")), "studied_college":str(row.get("Studied_College","")),
            "department":str(row.get("Department","")), "semester":str(row.get("Semester","")),
            "cgpa":float(row.get("CGPA")) if str(row.get("CGPA","")).strip() else None,
            "university_name":str(row.get("University_Name","")),
        })
    except Exception as exc:
        raise RuntimeError(f"Supabase Smart Card save failed: {exc}")
    try:
        df=load_smart_cards(); reg_id=str(row.get("Registration_ID","")).strip()
        if reg_id and not df.empty: df=df[df["Registration_ID"].astype(str).str.strip()!=reg_id]
        pd.concat([df,pd.DataFrame([row],columns=SMART_CARD_COLUMNS)],ignore_index=True).to_csv(SMART_CARD_FILE,index=False)
    except Exception: pass
