...
def create_principal_salary_request(sb, account, amount, salary_month, reference=""):
    """Create a pending salary request for Principal approval.

    This does NOT change the tutor balance. The balance changes only after
    the Principal presses APPROVE & CREDIT.
    """
    email = str(account.get("email") or "").strip().lower()
    name = str(account.get("tutor_name") or "Tutor").strip()
    amount = round(float(amount or 0), 2)
    month = str(salary_month or "").strip()
    if not email:
        raise ValueError("Tutor salary account email is missing.")
    if amount <= 0:
        raise ValueError("Salary amount must be greater than ₹0.")
    if not month:
        raise ValueError("Salary month is required.")

    ref = str(reference or "").strip() or f"Principal salary upload | {month}"
    sb.table("tutor_salary_transactions").insert({
        "tutor_id": email,
        "tutor_name": name,
        "amount": amount,
        "salary_month": month,
        "sent_by": PRINCIPAL_USERNAME,
        "reference": ref,
        "status": "pending",
    }).execute()
    return True
...
def approve_tutor_salary(sb, transaction):
    """Approve ONE pending tutor salary request and credit the tutor wallet safely."""
    txid = transaction.get("id")
    if txid is None:
        raise ValueError("Salary request ID is missing.")

    status = str(transaction.get("status") or "").strip().lower()
    if status != "pending":
        raise ValueError("This salary request is not pending. It may already be credited.")

    email = str(transaction.get("tutor_id") or "").strip().lower()
    tutor_name = str(transaction.get("tutor_name") or "").strip()
    amount = round(float(transaction.get("amount") or 0), 2)
    if not email or amount <= 0:
        raise ValueError("Invalid tutor salary account or salary amount.")

    # Load the tutor salary account.
    account_rows = rows(
        sb.table("tutor_accounts")
        .select("*")
        .eq("email", email)
        .limit(1)
        .execute()
    ...
    if account.get("active") is False:
        raise ValueError("This tutor salary account is inactive.")

    old_balance = round(float(account.get("balance") or 0), 2)
    new_balance = round(old_balance + amount, 2)

    # First credit the wallet.
    sb.table("tutor_accounts").update({"balance": new_balance}).eq("email", email).execute()

    # Then mark THIS exact pending request as credited.
    updated_rows = rows(
    ...
    # Never leave the wallet credited if the transaction could not be changed.
    if not updated_rows:
        sb.table("tutor_accounts").update({"balance": old_balance}).eq("email", email).execute()
        raise ValueError("Salary request was already processed or could not be marked as credited.")

    # Record the approval for Principal audit/history.
    try:
        sb.table("audit_logs").insert({
            ...
            "details": (
                f"Tutor={tutor_name or account.get('tutor_name','')}; "
                f"Email={email}; Amount=₹{amount:.2f}; "
        ...
        # Salary is already correctly credited; audit failure should not undo a valid payment.
        pass
...
def dashboard():
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
        "📅 Date-wise Records", "👨‍🏫 Live Tutor Activity", "💰 Salary Upload & Approval", "🪪 Smart Cards", "📁 Academic Folders", "🗑️ Delete Student", "🧹 Delete All History"
    ])
    ...
    with tab3:
        ...
        st.divider()
        st.subheader("📤 Upload Tutor Salary for Approval")
        st.caption("Principal can upload a salary amount to a tutor salary account. Upload creates a PENDING request only. The tutor balance changes after approval.")
        if salary_accounts:
            account_labels = {
                f"{a.get('tutor_name','Tutor')} — {a.get('email','')}": a
                for a in salary_accounts
            }
            selected_account_label = st.selectbox(
                "Select Tutor Salary Account",
                list(account_labels.keys()),
                key="principal_salary_upload_account",
            )
            selected_account_for_upload = account_labels[selected_account_label]
            c1, c2 = st.columns(2)
            with c1:
                upload_amount = st.number_input(
                    "Salary Amount (₹)",
                    min_value=1.0,
                    value=100.0,
                    step=100.0,
                    key="principal_salary_upload_amount",
                )
            with c2:
                upload_month = st.text_input(
                    "Salary Month",
                    value=datetime.now().strftime("%Y-%m"),
                    key="principal_salary_upload_month",
                )
            upload_reference = st.text_input(
                "Reference / Description",
                value="Principal salary upload",
                key="principal_salary_upload_reference",
            )
            if st.button("📤 UPLOAD SALARY FOR APPROVAL", type="secondary", use_container_width=True):
                try:
                    create_principal_salary_request(
                        sb, selected_account_for_upload, upload_amount, upload_month, upload_reference
                    )
                    st.success("✅ Salary uploaded successfully as PENDING. Principal approval is required before the tutor balance changes.")
                    st.rerun()
                except Exception as exc:
                    st.error(f"❌ Salary upload failed: {exc}")
        else:
            st.warning("No Tutor Salary Accounts found. Ask the tutor to create the separate salary account from the 👤 button on the Tutor Login page.")
